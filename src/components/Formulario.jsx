...
const Formulario=()=>{
        ...
        for(let i=0;i<res.data.length;i++){
            if(res.data[i].email===email){
                if(res.data[i].contr===password){
                    let token,username;
           try {
            const loginResponse = await axios.post('http://localhost:8000/us/api/v1/iniciar_sesion/', {
              "username": email,
              "password": password,
            });
 ...
 await axios.post(`http://localhost:8000/sesi/api/v1/sesi/`,{
  'hora':fecha.getHours()+":"+fecha.getMinutes()+":"+fecha.getSeconds(),
  'fecha':fecha.getFullYear()+'-'+month +'-'+fecha.getDate(),
  'email':username,
  'accion':'Iniciar Sesión',
 })
          await axios.post('http://localhost:8000/sesion/api/v1/sesion/',{
               'idPer': res.data[i].id,
                ...
                'fechaCierre':fecha.getFullYear()+'-'+month +'-'+fecha.getDate(),
                    ...
                    if(res.data[i].equivocaciones>5){
                      ...
                      console.log()
                       ...
                       await axios.put(`http://localhost:8000/personas/api/v1/personas/${res.data[i].id}/`,{
