...
newLikes+=em+','
  ...
  try {
    ...
    await axios.put(`http://localhost:8000/publications/api/v1/publications/${publicacionesV[aux].id}/`, {
...
let io;
 if((pdfPar==""||pdfPar==null||pdfPar==''||pdfPar==undefined)&&modo=="expositor"){
  ...
  const ress=await fetch("http://localhost:8000/solicitud/api/v1/solicitud/",{
    method:"POST",
    body:formData,
...
return true;
  ...
  return (
    ...
    <div className=' w-[101vw] mt-4  bg-[#7498b6] h-[2000vh] min-[1000px]:h-[280vh]  ja'>
      ...
      <h1 className='w-[10vw] max-w-[100px]  h-[40px]    text-center  ml-auto mr-auto relative min-[1000px]:left-[20rem] max-[1000px]:left-[15rem] min-w-[94.5px] max-[800px]:left-[10rem] max-[571px]:left-[8rem] max-[467px]:left-[6rem]'></h1>
...
<span   className='w-[20px] azulGusta ml-2 cursor-pointer '    onClick={(e) => Megusta(e)}>Me gusta</span> 
 ...
 <div className="container inline cursor-pointer mr-[2vw] "onClick={(e)=>Comentarios(e)} >
  <img src="../../images/comentario.png"    className='max-[1129px]:block max-h-[50px] h-[200px]  block xl:inline '  ></img>
...
<span  className='ml-1 cursor-pointer  block xl:inline'>Descargar</span>
 ...
 <div className="comp container block xl:inline cursor-pointer  max-[1129px]:block">                <img src="../../images/compartir.jpg"  className='comp max-h-[50px] block xl:inline cursor-pointer ' ></img> <span className='comp cursor-pointer ml-1 block xl:inline'>Compartir</span>
       ...
       <div className=''>
        ...
        {!ComentariosCardsd[publication.id] &&(
         ...
         {!EventosCardsBus[publication.id]&&!ComentariosCardsd[publication.id]&&(    <img
          src={`http://localhost:8000/static/${BusPdf(publication.foto)}`}
          alt="Imagen"
   ...
   {/*   {cantPubl>0 && !ComentariosCards &&(
    ...
    <h3 className='text-center mt-16 '>No hay Comentarios </h3>
      ...
      <span   className='w-[20px] azulGusta ml-2 cursor-pointer '    onClick={(e) => Megusta(e)}>Me gusta</span> 
                ...
                <a href={publicaciones.length > 0 ? `http://localhost:8000/static/${BusPdf(publication.pdf)}`:''} target='blank'  rel="noopener noreferrer" download="jornada_cientifica.pdf">
                  <img src="../../images/icon_download.jpg" className='max-h-[50px] inline' alt="" /></a>
               <span  className='ml-1 cursor-pointer'>Descargar</span>
                </div>
                <div className="container inline cursor-pointer">                <img src="../../images/compartir.jpg"  className='max-h-[50px] inline cursor-pointer'  onClick={()=>redVer(publication.id)}></img> <span className='cursor-pointer ml-1'>Compartir</span>
                </div>
                </div>)}
                <div className='bg-white '>
