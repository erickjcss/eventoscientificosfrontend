...
.catch(error => {
  console.error(error);
    ...
    /*   axios.get('http://localhost:8000/us/api/v1/usuario_activo/')
      .then(response => {
...
const shareUrl=`https://twitter.com/intent/tweet?text=Hola sea bienvenido a este Evento 🌟&url=http://localhost:5173/pages/eventos&via=Eventos Científicos&hastags=#eventos_cientificos`
  ...
  if(accepted){
    ...
    for(let i=0;i<autores.length;i++){
        ...
        if(autores[i].id==publicaciones[numPub].publicacionesPer){
          console.log('ja')
          try{
            await axios.put(`http://localhost:8000/personas/api/v1/personas/${autores[i].id}/`,{
                      ...autores[i],"cantNotificaciones":autores[i].cantNotificaciones+1                   })
            /* await axios.put(`http://localhost:8000/publications/api/v1/publications/${publicaciones[numPub].id}/`, { */
          }
         catch (error) {
          console.error('Error al obtener el nombre de usuario:', error);
      }
      try{
        await axios.post("http://localhost:8000/notifica/api/v1/notifica/",{
 ...
 let fecha =new Date();
        ...
        if (accepted) {
          for(let i=0;i<autores.length;i++){
            ...
            await axios.put(`http://localhost:8000/personas/api/v1/personas/68/`,{
      ...
      break;
            ...
            try {
                const res =  axios.put(`http://localhost:8000/publications/api/v1/publications/${publicacionesP[numPubP].id}/`, {
              ...
              if(accepted){
               ...
               for(let i=0;i<autores.length;i++){
                ...
                if(autores[i].id=publicaciones[numPub].publicacionesPer){
                  ...
                  await axios.post(`http://localhost:8000/us/api/v1/elAdm/`,{
...
<a href={publicaciones.length > 0 ? `http://localhost:8000/static/${cambDirPdfS(solicitud)}`:''} target='blank'  rel="noopener noreferrer" download="jornada_cientifica.pdf">
...
{/*         <div class="sharethis-inline-share-buttons"></div> */}
 ...
 <h2 className='text-center min:665px:text-4xl mr-[1.754rem] text-2xl bg-[transparent] min-[1500px]:text-6xl  min-[1500px]:mr-60  font-mono min:665px:mr-48  mb-8 xl:text-left '>Administra los Eventos Publicados</h2>
  ...
  <a href={publicaciones.length > 0 ? `http://localhost:8000/static/${cambDirPdf(numPub)}`:''} target='blank'  rel="noopener noreferrer" download="jornada_cientifica.pdf">
...
<section id='sliderP' className='mt-16  ml-4 mr-4 slider relative top-[1000px] max-[340px]:top-[900px] min-[1500px]:top-[1000px]'>
      ...
      <h2 className='text-center min:665px:text-4xl mr-[1.754rem] text-2xl bg-[transparent] min-[1500px]:text-6xl  min-[1500px]:mr-60  font-mono min:665px:mr-48  mb-8 '>Eventos Pendientes</h2>
        ...
        <img id='fotoObtener'
          src={`http://localhost:8000/static/${cambDirFotoP(numPubP)}`}
          alt="Imagen"
...
<div className='fotos absolute inline-flex  min-h-[75px] justify-start    border-t  w-[inherit] border-orange-400 bg-white min-w-[400px] max-[400px]:min-w-[100vw]'>
      ...
      <span   className='w-[20px] azulGusta ml-2 cursor-pointer hover:text-green-300'  >Aprobar</span> 
                ...
                <div className="container inline cursor-pointer">
                <a href={publicacionesP.length > 0 ? `http://localhost:8000/static/${cambDirPdfP(numPubP)}`:''} target='blank'  rel="noopener noreferrer" download="jornada_cientifica.pdf">
...
<section id='sliderP' className='max-w-[650px] min-[870px]:ml-[2vw] min-[900px]:ml-auto min-[900px]:mr-auto mt-16 w-[100vw] min-[600px]:w-[85vw] min-[600px]:ml-[4vw] slider relative top-[2000px] max-h-[800px]  overflow-auto scroll-auto bg-slate-400 est min-[1300px]:min-w-[900px]' >
  ...
  <a className='relative min-[600px]:mr-24 min-[700px]:mr-20 min-[800px]:mr-4' href="http://localhost:8000/admin/" target='blank'> <button className='relative bg-[#557986] cursor-pointer   w-[133%]  min-[1300px]:left-[12.6rem] hover:text-white'> Base de datos</button></a>
  </div>
